# Flash Card App - Language learning app with spaced repetition
from tkinter import *
import pandas
import random
...
# Load word data, prioritizing words to learn if file exists
try:
    data = pandas.read_csv("./data/words_to_learn.csv")
except FileNotFoundError:
    # Fall back to original word list if learning file doesn't exist
    original_data = pandas.read_csv("./data/french_words.csv")
    data_dict = original_data.to_dict(orient="records")
except ValueError:
    # Handle empty or corrupted learning file
    original_data = pandas.read_csv("./data/french_words.csv")
    data_dict = original_data.to_dict(orient="records")
else:
...
def next_card():
    ...
    window.after_cancel(flip_timer)  # Reset auto-flip timer
    current_card = random.choice(data_dict)
    # Display French side of card
    ...
    flip_timer = window.after(3000, flip_card)  # Set auto-flip after 3 seconds

def flip_card():
    # Reveal English translation on back of card
...
flip_timer = window.after(3000, func=flip_card)  # Initial auto-flip timer

# Create flash card canvas
